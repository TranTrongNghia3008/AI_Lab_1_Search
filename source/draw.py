...
class Draw:
    # def __init__ (self, input_file, trace_file):
        # self.input_file = input_file
        # self.trace_file = trace_file

    # Create a list to store each frame
    def visualize_maze(self, matrix, bonus, start, end, route=None, traversal_cells=None , speed = 4):
        """
        Args:
        ...
        if traversal_cells:
            for i in range(len(traversal_cells) - 2):
                plt.scatter(traversal_cells[i + 1][1], -traversal_cells[i + 1][0],
                            marker='.', s=100, color='hotpink',edgecolors='none')
                if i % speed == 0 or i ==  len(traversal_cells) - 2:

                    plt.savefig('stop.png')
                    frames.append(imageio.imread('stop.png'))
                # plt.savefig('stop.png')
                # frames.append(imageio.imread('stop.png'))
    ...
    def remove_duplicates_keeping_order(self, arr):
    ...
    def read_traversal_and_route(self, file_name: str = 'traversal_and_route.txt'):
    ...
    def convert_result_to_video(self, input_file = "input.txt", trace_file = "output.txt",saved_file="maze.gif",speed = 3):
        bonus_points, matrix = self.read_file(input_file)
        traversal_cells, route = self.read_traversal_and_route(trace_file)

        total_number_of_openings = len(traversal_cells)
        traversal_cells = self.remove_duplicates_keeping_order(traversal_cells)
        ...
        frames = self.visualize_maze(matrix, bonus_points, start, end,route,traversal_cells,speed)
